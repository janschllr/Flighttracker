...
function getTzOffsetMinutes(timezone) {
  try {
    const now = new Date();
    const utcStr = new Intl.DateTimeFormat('en-US', {
      timeZone: 'UTC',
...
function getFlightDuration(depObj, arrObj, depTz, arrTz) {
  const depTime = getBestTime(depObj);
  const arrTime = getBestTime(arrObj);
  if (!depTime || !arrTime) return null;

  // API returns local airport times disguised as UTC — correct with tz offsets
